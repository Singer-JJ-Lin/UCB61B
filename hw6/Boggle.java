...
public class Boggle {
    ...
    public static List<String> solve(int k, String boardFilePath) {
        ...
        for(int i = 0; i < x * y; i++){
            ...
            p = trie.root;
            stack.push(p);
            if(!p.sons.containsKey(startChar)){
                continue;
            }
            passed.add(i);
            p = p.sons.get(startChar);
            depthFirstSearching(i, myMap,boardOneD);
        }
        return sortList(toReturn,k);
    }

    private static Tire trie;

    private static Stack<Tire.Node> stack;

    private static Tire.Node p;
    ...
    private static void depthFirstSearching(int start, List<Map<Integer, Character>> myMap, char[] M){
        ...
        for(int next : adjMap.keySet()){
            ...
            if(!p.sons.containsKey(nextChar)){
                continue;
            }
            p = p.sons.get(nextChar);
            passed.add(next);
